...
class Messages {
    ...
    class func startMultipleChat(users: [PFUser]!) -> String {
        ...
        let sorted = userIds.sortInPlace { $0.localizedCaseInsensitiveCompare($1) == NSComparisonResult.OrderedAscending }
            ...
            if error == nil {
                if objects.count == 0 {
                    ...
                    message.saveInBackgroundWithBlock({ (succeeded: Bool, error: NSError!) -> Void in
                        if (error != nil) {
                            print("Messages.createMessageItem save error.", terminator: "")
    ...
    class func deleteMessageItem(message: PFObject) {
        message.deleteInBackgroundWithBlock { (succeeded: Bool, error: NSError!) -> Void in
            if error != nil {
                print("UpdateMessageCounter save error.", terminator: "")
                print(error, terminator: "")
            }
        }
    }
        ...
        query.findObjectsInBackgroundWithBlock { (objects: [AnyObject]!, error: NSError!) -> Void in
            if error == nil {
                for message in objects as! [PFObject]! {
                    ...
                    message.saveInBackgroundWithBlock({ (succeeded: Bool, error: NSError!) -> Void in
                        if error != nil {
                            print("ClearMessageCounter save error.", terminator: "")
                            print(error, terminator: "")
                        }
                    })
                }
